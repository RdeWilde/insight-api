...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix ioncore to give back null instead of null hash
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.bitcoind.getRawBlock(hash, function(err, blockBuffer) {
      ...
      var br = new ioncore.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in ioncore-node.
      var header = ioncore.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [ioncore.Transaction().fromBufferReader(br)];
